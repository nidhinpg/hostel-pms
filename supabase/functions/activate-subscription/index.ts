...
Deno.serve(async (req) => {
  ...
  const { property_id, plan_type, razorpay_payment_id, razorpay_subscription_id, razorpay_signature } = await req.json()
  ...
  const { error } = await supabase.from('properties')
    .update({ plan_type: plan_type || 'pro', subscription_status: 'active', razorpay_subscription_id })
    .eq('id', property_id)

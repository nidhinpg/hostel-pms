...
// ─── Settings Modal ───────────────────────────────────────────────────────────
function SettingsModal({ onClose, activeProperty, onSaved, onOpenStaffManagement, onOpenAddProperty, showAddProperty, onOpenManageProperties }) {
...
// Lists every property the owner has, its GPay number, and a delete button.
// Delete is blocked server-side if the property still has tenants/beds, or if
// it's the owner's only remaining property.
function ManagePropertiesModal({ onClose, userId, properties, activeProperty, onDeleted }) {
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20 }} onClick={onClose}>
      <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', padding: 24, minWidth: 340, maxWidth: 440, width: '100%', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
          {properties.map(p => (
            <div key={p.id} style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: activeProperty?.id === p.id ? 'var(--bg)' : 'transparent' }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600 }}>{p.name}</div>
                <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>
                  {p.city ? p.city + ' · ' : ''}GPay: {p.gpay_number || '—'}
                </div>
              </div>
              <button
                onClick={() => handleDelete(p)}
                disabled={deletingId === p.id || properties.length <= 1}
                title={properties.length <= 1 ? 'You must keep at least one property' : 'Delete property'}
                style={{ fontSize: 11, color: properties.length <= 1 ? 'var(--text-tertiary)' : 'var(--red)', background: 'none', border: 'none', cursor: properties.length <= 1 ? 'not-allowed' : 'pointer', padding: '4px 8px', textDecoration: 'underline', flexShrink: 0 }}>

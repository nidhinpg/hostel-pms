    ...
    <div>
      ...
      <div className="card">
        {loading ? <div className="loading">Loading...</div> : transactions.length === 0 ? (
          <div className="empty">No entries for {month}</div>
        ) : (
        ...
        <Modal title="Add entry" onClose={() => setShowAdd(false)}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div className="form-grid single">
              <div className="form-group"><label>Description</label><input placeholder="Short note" value={form.description} onChange={f('description')} /></div>
            </div>
          </div>
        </Modal>
      )}

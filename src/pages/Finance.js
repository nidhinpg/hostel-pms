  ...
  return (
    <div>
      ...
      <div className="summary-pills">
        ...
        <span className="pill" style={{ background: 'var(--blue-bg)', color: 'var(--blue)' }}>{transactions.length} entries</span>
      </div>

      <div className="card">
        {loading ? <div className="loading">Loading...</div> : transactions.length === 0 ? (
          <div className="empty">No entries for {month}.<br /><span style={{ fontSize: 12, marginTop: 4, display: 'block' }}>Click "+ Add entry" to record income or expenses.</span></div>
        ) : (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div className="form-grid">
              ...
              <div className="form-group"><label>Type</label>
                <select value={form.type} onChange={f('type')}><option value="income">Income</option><option value="expense">Expense</option></select>
              </div>
            </div>
            <div className="form-grid">
            ...
            <div className="form-grid single">
              <div className="form-group"><label>Description (optional)</label><input placeholder="e.g. Rahul - April rent" value={form.description} onChange={f('description')} /></div>
            </div>
          </div>
        </Modal>
      )}
